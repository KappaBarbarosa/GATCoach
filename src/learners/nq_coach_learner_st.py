...
class NQ_Coach_Learner_ST:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.coach = REGISTRY[args.coach](args)
        self.target_coach = copy.deepcopy(self.coach)
        self.params +=  list(self.coach.parameters())
        if self.args.use_vi:
            self.vi = VI(args)
            self.params += list(self.vi.parameters())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        elbo = 0.
        if self.args.use_vi:
            vi_loss = self.vi(batch, strategies)
            p_ = D.normal.Normal(means, (0.5 * variances).exp())
        ...
        if self.args.coach == "embedding":
            diversity_loss = self.coach.loss()
            diversity_loss = diversity_loss * self.args.embedding_loss_weight

        # Optimise
        self.optimiser.zero_grad()
        
        (loss + elbo + diversity_loss).backward()
    ...
    def save_models(self, path):
        ...
        th.save(self.coach.state_dict(), "{}/coach.th".format(path))
        if self.mixer is not None:
    ...
    def load_models(self, path):
        ...
        self.coach.load_state_dict(th.load("{}/coach.th".format(path), map_location=lambda storage, loc: storage))
