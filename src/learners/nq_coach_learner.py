...
class NQ_Coach_Learner:
    def __init__(self, mac, scheme, logger, args):
        ...
        print('Coach Size: ')
        print(get_parameters_num(coach_params) )

        if self.args.optimizer == 'adam':
            self.optimiser = Adam(params=self.params,  lr=args.lr, weight_decay=getattr(args, "weight_decay", 0))
            self.coach_optimizer = Adam(params=self.coach_params, lr=args.lr, weight_decay=getattr(args, "weight_decay", 0))
        else:
            self.optimiser = RMSprop(params=self.params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
            self.coach_optimizer = RMSprop(params=self.coach_params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        training_team_strategy = self.mac.agent.z_team.clone()  # save previous team strategy

        z_t0, mu_t0, logvar_t0 = self.coach(batch,t=0,isinference=False)
        z_t0_target, _, _ = self.target_coach(batch,t=0,isinference=True)
        z_T_target, _, _ = self.target_coach(batch,t=T-1,isinference=True)

        self.mac.agent.set_team_strategy(z_t0)
        self.target_mac.agent.set_team_strategy(z_t0_target)
        ...
        with th.no_grad():
            ...
            for t in range(batch.max_seq_length):
                if t==batch.max_seq_length-1:
                    self.target_mac.agent.set_team_strategy(z_T_target)
                target_agent_outs = self.target_mac.forward(batch, t=t)
                target_mac_out.append(target_agent_outs)
                target_caoch_h = self.target_coach.encode(batch,t=t)
                target_coach_out.append(target_caoch_h)

            # We don't need the first timesteps Q-Value estimate for calculating targets
            target_mac_out = th.stack(target_mac_out, dim=1)  # Concat across time
            target_coach_out = th.stack(target_coach_out, dim=1)
            ...
            target_max_qvals = self.target_mixer.coach_forward(target_max_qvals,target_coach_out, batch["state"])

            if getattr(self.args, 'q_lambda', False):
                qvals = th.gather(target_mac_out, 3, batch["actions"]).squeeze(3)
                qvals = self.target_mixer.coach_forward(qvals,target_coach_out, batch["state"])
        ...
        # Mixer
        chosen_action_qvals = self.mixer.coach_forward(chosen_action_qvals,coach_out, batch["state"][:, :-1])
        ...
        self.mac.agent.set_team_strategy(training_team_strategy)
